...
const AnniversaryEditPosterPopup = ({
    isOpen,
    onClose,
    footerStyle,
    setFooterStyle,
    ...
    useEffect(() => {
        ...
        if (avatarOverlays.length > 0 || textOverlays.length > 0) {
            console.log("🛠️ [AnniversaryEditor] Overlays already initialized, skipping reset.");
            return;
        }

        console.log("🛠️ [AnniversaryEditor] Initializing avatar overlays from postData", postData._id || postData.id);
        ...
        console.log("🛠️ [AnniversaryEditor] Initializing text overlays");
    ...
    const handleAvatarUpdate = useCallback((newOverlays) => {
        console.log("🛠️ [AnniversaryEditor] Updating avatar overlays", newOverlays.length);
    ...
    const addNewAvatar = () => {
        console.log("🛠️ [AnniversaryEditor] Adding new manual avatar slot");
        ...
        setAvatarOverlays(prev => {
            ...
            console.log("🛠️ [AnniversaryEditor] New overlay count:", next.length);
            return next;
        });
    ...
    const handleTextUpdate = useCallback((newOverlays) => {
        console.log("🛠️ [AnniversaryEditor] Updating text overlays", newOverlays.length);
        setTextOverlays(newOverlays);
    }, []);

    const addNewText = () => {
        console.log("🛠️ [AnniversaryEditor] Adding new manual text slot");
                                            ...
                                            <h3 className="text-xl font-bold text-gray-900 line-clamp-1">
                                                {currentView === 'avatarList' ? 'Select Avatar Slot' :
                                                    currentView === 'avatarEdit' ? 'Edit Photo & Shape' :
                                                        currentView === 'textEdit' ? 'Text Style & Size' : 'Anniversary Editor'}
                                            </h3>
                                        )}
                                        {currentView === 'root' && (
                                            <h3 className="text-xl font-bold text-gray-900">Anniversary Editor</h3>
                                        )}
                                        {isMobile && (
...
export default AnniversaryEditPosterPopup;
