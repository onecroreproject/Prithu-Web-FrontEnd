...
const AnniversaryEditPosterPopup = ({
    ...
    // Add ref to track if we're in the middle of an update (prevents re-render lag)
    const isUpdatingFromDrag = useRef(false);

    // Coordinate logging (for developer verification)
    useEffect(() => {
        if (avatarOverlays.length > 0) {
            console.log("📍 [AnniversaryCoordinateSync] Avatars:", avatarOverlays.map(a => `(${a.x}, ${a.y})`));
        }
    }, [avatarOverlays]);

    useEffect(() => {
        if (!isOpen) {
            // Reset state when closing/closed
            setAvatarOverlays([]);
            setTextOverlays([]);
            setCurrentView('root');
        ...
        console.log("🛠️ [AnniversaryEditor] Initializing fresh overlays with (10,10) defaults");
        ...
        const textEls = postData.overlayElements
            ...
            .map((el, idx) => ({
                ...
                style: el.textConfig || el.style || { fontSize: 24, color: '#ffffff', fontFamily: 'Inter', fontWeight: 'bold', align: 'center' },
                                ...
                                {isMobile && (
                                    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white z-[100]">
                                        <h3 className="text-lg font-bold text-gray-900">
                                            {currentView === 'root' ? 'Anniversary Editor' :
