...
const JobLayout = () => {
  ...
  const [allJobs, setAllJobs] = useState([]); // Store unfiltered jobs for accurate filter counts
 ...
 const fetchJobs = async () => {
  ...
  setLoading(true);
  setFadeOut(true);
  // Only scroll to results if this is not the initial load
  if (hasAttemptedLoad) {
  ...
  try {
    ...
    // Note: employmentType, workMode, and salaryRange filters are handled client-side only
    // to ensure consistent filtering across all job listings
    ...
    // Store unfiltered jobs for accurate filter counts
    setAllJobs(jobsFromApi);

    // Apply client-side filtering as fallback
    let filteredJobs = jobsFromApi;
    
    // Client-side experience filtering - FIXED OVERLAP LOGIC
    if (filters.experience) {
      console.log("Applying client-side experience filter:", filters.experience);
      filteredJobs = filteredJobs.filter(job => {
        // Get experience value from job
        let minExp = 0;
        let maxExp = Infinity; // Default to infinity if not specified

        // Parse experience from job data
        if (job.minimumExperience !== undefined && job.minimumExperience !== null) {
          minExp = parseFloat(job.minimumExperience) || 0;
        }
        if (job.maximumExperience !== undefined && job.maximumExperience !== null) {
          maxExp = parseFloat(job.maximumExperience) || Infinity;
        }

        // If no experience specified, check if freshers allowed
        if (minExp === 0 && maxExp === Infinity) {
          if (job.freshersAllowed === true) {
            maxExp = 1; // Freshers can apply
          } else {
            return true; // No experience restriction, include job
          }
        }

        // Match the selected experience range using overlap logic
        // Job range [minExp, maxExp] should overlap with filter range
        switch(filters.experience) {
          case "0": // Fresher (0-1 yrs) - filter range [0, 1]
            return maxExp >= 0 && minExp <= 1;
          case "1-3": // 1-3 years - filter range [1, 3]
            return maxExp >= 1 && minExp <= 3;
          case "3-5": // 3-5 years - filter range [3, 5]
            return maxExp >= 3 && minExp <= 5;
          case "5-8": // 5-8 years - filter range [5, 8]
            return maxExp >= 5 && minExp <= 8;
          case "8+": // 8+ years - filter range [8, Infinity]
            return minExp >= 8;
          default:
            return true;
        }
    ...
    // Client-side filtering for employmentType
    if (filters.employmentType.length > 0) {
      console.log("Applying client-side employmentType filter:", filters.employmentType);
      filteredJobs = filteredJobs.filter(job => {
        if (!job.employmentType) return false;
        const jobType = job.employmentType.toLowerCase().trim();
        return filters.employmentType.some(type => {
          const filterType = type.toLowerCase().trim();
          return jobType.includes(filterType) ||
                 jobType.includes(filterType.replace('-', ' ')) ||
                 jobType.includes(filterType.replace(' ', '-'));
        });
      });
      console.log("After employmentType filtering:", filteredJobs.length, "jobs");
    }

    // Client-side filtering for workMode
    if (filters.workMode.length > 0) {
      console.log("Applying client-side workMode filter:", filters.workMode);
      filteredJobs = filteredJobs.filter(job => {
        if (!job.workMode) return false;
        const jobMode = job.workMode.toLowerCase().trim();
        return filters.workMode.some(mode => {
          const filterMode = mode.toLowerCase().trim();
          return jobMode.includes(filterMode) ||
                 jobMode.includes(filterMode.replace('-', ' ')) ||
                 jobMode.includes(filterMode.replace(' ', '-'));
        });
      });
      console.log("After workMode filtering:", filteredJobs.length, "jobs");
    }

    // Client-side filtering for salaryRange
    if (filters.salaryRange) {
      console.log("Applying client-side salaryRange filter:", filters.salaryRange);
      filteredJobs = filteredJobs.filter(job => {
        if (!job.salaryRange && !job.salaryMin && !job.salaryMax && !job.minimumSalary && !job.maximumSalary) return false;

        // Try to get salary value from different fields (matching JobCards component field names)
        let salaryValue = 0;
        if (job.salaryMin) {
          salaryValue = parseFloat(job.salaryMin);
        } else if (job.salaryMax) {
          salaryValue = parseFloat(job.salaryMax);
        } else if (job.minimumSalary) {
          salaryValue = parseFloat(job.minimumSalary);
        } else if (job.maximumSalary) {
          salaryValue = parseFloat(job.maximumSalary);
        } else if (job.salaryRange) {
          // Parse salary range string like "3-6 LPA" or "50000-80000"
          const salaryStr = job.salaryRange.toString();
          const numbers = salaryStr.match(/\d+(\.\d+)?/g);
          if (numbers && numbers.length > 0) {
            salaryValue = parseFloat(numbers[0]);
            // Convert to LPA if it's in rupees
            if (salaryValue > 1000) {
              salaryValue = salaryValue / 100000; // Convert rupees to LPA
            }
          }
        }
        ...
        switch(filters.salaryRange) {
          case "0-3":
            return salaryValue >= 0 && salaryValue < 3;
          case "3-6":
            return salaryValue >= 3 && salaryValue < 6;
          case "6-10":
            return salaryValue >= 6 && salaryValue < 10;
          case "10-15":
            return salaryValue >= 10 && salaryValue < 15;
          case "15-25":
            return salaryValue >= 15 && salaryValue < 25;
          case "25+":
            return salaryValue >= 25;
          default:
            return true;
        }
      });
      console.log("After salaryRange filtering:", filteredJobs.length, "jobs");
    }
  ...
  }, [selectedState, selectedCity, selectedArea, selectedCountry, searchText, filters, cityParam, countryParam, companyParam, jobIdParam, userLocation, distanceRadius]);

  // Fetch jobs when filters change (after initial load)
  useEffect(() => {
    if (!isFirstRender.current && hasAttemptedLoad) {
      fetchJobs();
    }
  }, [filters, selectedState, selectedArea, selectedCountry, userLocation, distanceRadius, selectedCategory, jobTitleSearch]);

  // Only fetch jobs on initial load or when URL parameters change
  useEffect(() => {
  ...
  // Fetch jobs when URL parameters change (for direct links)
  useEffect(() => {
    if (!isFirstRender.current && (roleParam || jobTitleParam || cityParam || stateParam || countryParam || companyParam || latParam || lngParam || radiusParam)) {
      fetchJobs();
    }
  }, [roleParam, jobTitleParam, cityParam, stateParam, countryParam, companyParam, latParam, lngParam, radiusParam]);

  // Scroll to top on initial load, or to results if filters are present
  useEffect(() => {
    const hasInitialFilters = roleParam || jobTitleParam || cityParam || stateParam || countryParam || companyParam || latParam || lngParam || radiusParam;
    if (hasInitialFilters) {
      // Scroll to job results section if there are initial filters
      setTimeout(() => {
        jobCardsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 100);
    } else {
      // Scroll to top if no initial filters
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  }, []);

  // Scroll to job cards when a search or filter is applied
  useEffect(() => {
