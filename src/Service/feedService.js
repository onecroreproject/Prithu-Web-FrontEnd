...
// Helper to normalize individual feed items
const normalizeFeedItem = (feed, viewer = null, viewerSocialLinks = []) => {
...
export const getAllFeeds = async (page = 1, token, categoryId = null, postType = null) => {
  try {
    ...
    if (!feedsArray || !Array.isArray(feedsArray)) return [];
    ...
    return feedsArray.map(feed => normalizeFeedItem(feed, viewer, viewerSocialLinks));
  } catch (error) {
...
export const getSingleFeed = async (id, token) => {
  try {
    ...
    // viewer data might be in data.data.viewer or data.viewer
    const viewer = data.viewer || data.data?.viewer;
    return normalizeFeedItem(feedData, viewer);
  } catch (error) {
    console.error("❌ [getSingleFeed] Error:", error.message);
    return null;
  }
};
...
export const getTrendingFeeds = async (page = 1, token, postType = null) => {
  try {
    ...
    const feedsArray = data?.data?.feeds || (Array.isArray(data?.data) ? data.data : []);
    const viewer = data?.data?.viewer || null;
